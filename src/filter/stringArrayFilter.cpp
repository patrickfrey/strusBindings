 ...
 */
/// \file stringArrayFilter.cpp
#include "stringArrayFilter.hpp"
...
stringArrayFilter::stringArrayFilter( const stringArrayFilter& o)
	:m_impl(o.m_impl),m_ownership(o.m_ownership),m_state(o.m_state)
{
	std::memcpy( m_index, o.m_index, sizeof(m_index));
}

stringArrayFilter::stringArrayFilter( const std::vector<std::string>* impl)
	:m_impl(impl),m_ownership(0),m_state(1)
{
	std::memset( m_index, 0, sizeof(m_index));
}

stringArrayFilter::stringArrayFilter( std::vector<std::string>* impl, bool withOwnership)
...
static bindings::ValueVariant getElementValue( const std::vector<std::string>& elem, int valueIndex)
{
	switch (valueIndex) {
...
BindingFilterInterface::Tag stringArrayFilter::getNext( bindings::ValueVariant& val)
{
...
void stringArrayFilter::skip()
{
	const filter::StateTable::Element& st = g_struct_statetable[ m_state];
	m_state = st.skipState;
}

BindingFilterInterface* stringArrayFilter::createCopy() const
{
	return new stringArrayFilter(*this);
}

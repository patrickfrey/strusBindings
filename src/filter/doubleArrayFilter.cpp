 ...
 */
/// \file doubleArrayFilter.cpp
#include "doubleArrayFilter.hpp"
...
doubleArrayFilter::doubleArrayFilter()
	:m_impl(0),m_ownership(0),m_state(0)
{
	std::memset( m_index, 0, sizeof(m_index));
}

doubleArrayFilter::doubleArrayFilter( const doubleArrayFilter& o)
	:m_impl(o.m_impl),m_ownership(o.m_ownership),m_state(o.m_state)
{
	std::memcpy( m_index, o.m_index, sizeof(m_index));
}

doubleArrayFilter::doubleArrayFilter( const std::vector<double>* impl)
	:m_impl(impl),m_ownership(0),m_state(1)
{
	std::memset( m_index, 0, sizeof(m_index));
}

doubleArrayFilter::doubleArrayFilter( std::vector<double>* impl, bool withOwnership)
...
static bindings::ValueVariant getElementValue( const std::vector<double>& elem, int valueIndex)
{
	switch (valueIndex) {

		case 0:
			return bindings::ValueVariant( (*m_impl)[m_index[0]]);

	}
	return bindings::ValueVariant();
}

BindingFilterInterface::Tag doubleArrayFilter::getNext( bindings::ValueVariant& val)
{
...
void doubleArrayFilter::skip()
{
	const filter::StateTable::Element& st = g_struct_statetable[ m_state];
	m_state = st.skipState;
}

BindingFilterInterface* doubleArrayFilter::createCopy() const
{
	return new doubleArrayFilter(*this);
}
